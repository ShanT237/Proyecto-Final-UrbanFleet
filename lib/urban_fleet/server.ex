defmodule UrbanFleet.Server do
  ...
  defp process_remote_command("request_trip " <> args, %{role: :client} = user) do
    case parse_trip_args(args) do
      ...
      :error ->
        {:error, "✗ Uso: request <origen> <destino>  (o request_trip origen=.. destino=.. )", user}
    end
  end
  ...
  defp show_client_help do
    ...
    ║           📱 COMANDOS CLIENTE           ║
    ╚════════════════════════════════════════╝
    request <origen> <destino>              - Solicitar viaje (forma corta)
    ...
    case input do
      ...
      raw ->
        ...
        case process_server_command(cmd) do
          ...
          :exit ->
            IO.puts("🖥️ Servidor detenido (CLI finalizado).")
            System.halt(0)
        end
    end
  end
